...
public class Main extends ApplicationAdapter {
    ...
    public void render() //runs every frame, used for rednering
    {
        ...
        if (frogger.isDead) { // Assuming `isDead()` checks the frog's death state
            frogger.deadTimer -= deltaTime;
            if (frogger.deadTimer <= 0)
            {
                resetGame();
            }
            return; // Exit this frame after resetting
        }
    ...
    */
    ...
    //restarts the game
    //its 2:00am to lazy to do this correctly
    public void resetGame() {
        ...
        tileMapRenderer.dispose();
        music.dispose();
        for (Sound sound : destroySounds) {
            sound.dispose();
        }
    
        //reinitialize the game
        create(); //call create to reset all states
    }
}
